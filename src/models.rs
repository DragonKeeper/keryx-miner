...
pub const KIMI_LINEAR_48B: ModelSpec = ModelSpec {
    ...
    // Same gate as the outgoing 70B-Q2 so the top tier stays 5090-class.
    min_vram_mb: 30_000,
};
...
pub fn pom_tier_index(model_id: &[u8; 32], daa: u64) -> Option<u8> {
    ...
    } else if daa >= VERY_LIGHT_ACTIVATION_DAA {
...
pub fn staging_daa() -> u64 {
    if h4_staged() {
        ...
        VERY_LIGHT_ACTIVATION_DAA
    }
}

/// True once the H4 hardfork has a scheduled DAA — startup staging (lineup + VRAM ladder)
